...
public class ViewUserUpdate {
    ...
    // These are the alerts to be used when invalid input is given
    protected static Alert alertEmailAddressError = new Alert(AlertType.INFORMATION);
	...
	private ViewUserUpdate() {
        ...
        button_UpdatePassword.setOnAction((_) -> {
        	// Loop until user inputs valid email or presses cancel
        	while (true) {
            	// Get password input from user
        		result = dialogUpdatePassword.showAndWait();
            	// Call password recognizer to check the input's validity
            	
        		String passwordCheck = PasswordRecognizer.checkForValidPassword(result.get());
        		System.out.println("checking password: " + passwordCheck);
            	
        		// If password isn't valid tell the user and refuse to continue
        		if(passwordCheck.compareTo("") != 0) {
        			// Error message sent to user
        			System.out.println(passwordCheck + " : " + result.get());
        			alertPasswordError.setTitle("*** WARNING ***");
        			alertPasswordError.setHeaderText("User Update Issue");
        			alertPasswordError.setContentText(passwordCheck);
        			alertPasswordError.showAndWait();
        		}
        		// Else continue like normal and save the new Email
        		else {
        ...
        button_UpdateEmailAddress.setOnAction((_) -> {
        	...
        	while (true) {
            	...
            	// Call email address recognizer to check the input's validity
            	String emailCheck = EmailAddressRecognizer.checkEmailAddress(result.get());
        		// If email isn't valid tell the user and refuse to continue
        		if(emailCheck.compareTo("") != 0) {
        			// Error message sent to user
        			System.out.println(emailCheck + " : " + result.get());
        			alertEmailAddressError.setTitle("*** WARNING ***");
        			alertEmailAddressError.setHeaderText("User Update Issue");
        			alertEmailAddressError.setContentText(emailCheck);
        			alertEmailAddressError.showAndWait();
        		}
        		// Else continue like normal and save the new Email
        		else {
