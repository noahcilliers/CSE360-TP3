 ...
 */
	public void connectToDatabase() throws SQLException {
		...
		try {
		    statement.execute("ALTER TABLE userPosts ADD COLUMN numericGrade INT DEFAULT -1");
		} catch (SQLException e) {
		    // column probably already exists
		}

		try {
		    statement.execute("ALTER TABLE userPosts ADD COLUMN letterGrade VARCHAR(20) DEFAULT 'Not Assigned'");
		} catch (SQLException e) {
		    // column probably already exists
		}
	}
 ...
 */
	private void initPostList() throws SQLException {
	    ...
	    while (resultSet.next()) {
	        ...
	        int numericGrade = resultSet.getInt("numericGrade");
	        String letterGrade = resultSet.getString("letterGrade");
	        ...
	        if (letterGrade == null || letterGrade.isBlank()) {
	            p.setLetterGrade("Not Assigned");
	        } else {
	            p.setLetterGrade(letterGrade);
	        }
 ...
 */
	private void createTables() throws SQLException {
        ...
        String userPostsTable = "CREATE TABLE IF NOT EXISTS userPosts ("
        		...
        		+ "numericGrade INT DEFAULT -1, "
        		+ "letterGrade VARCHAR(20) DEFAULT 'Not Assigned')";
 ...
 */	
	...
	/*******
	 * <p> Method: boolean updatePostGrade(int postId, int numericGrade, String letterGrade) </p>
	 * 
	 * <p> Description: Updates the numeric and letter grade for a specific post in the database. </p>
	 * 
	 * @param postId The unique ID of the post to update
	 * @param numericGrade The numeric grade to assign (0–100)
	 * @param letterGrade The corresponding letter grade (A–F)
	 * 
	 * @return true if the post was found and updated successfully, else false
	 */
	public boolean updatePostGrade(int postId, int numericGrade, String letterGrade) {
	    for (Post p : postList) {
	        if (p.getPostId() == postId) {
	            ...
	            try {
	                String sql = "UPDATE userPosts SET numericGrade = ?, letterGrade = ? WHERE postId = ?";
	                PreparedStatement pstmt = connection.prepareStatement(sql);
	                pstmt.setInt(1, numericGrade);
