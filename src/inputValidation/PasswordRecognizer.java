...
public class PasswordRecognizer {
	...
	private static boolean running;						// The flag that specifies if the FSM is 
														// running
	private static int passwordSize = 0;			// A numeric value may not exceed 16 characters
	...
	/**********
	 * This method simply checks if the password passed as input has the desired length.
	 * 
	 * 
	 * @param input		The input string for the size checks
	...
	public static String checkForValidPassword(String input) {
		...
		if (input.length() >= 256) {
			passwordRecognizerErrorMessage = "A Password must have no more than 255 characters.\n";
			return passwordRecognizerErrorMessage;
		}
		else if(input.length() <= 7)  {
			passwordRecognizerErrorMessage = "A Password must have at least 8 characters.\n";
			return passwordRecognizerErrorMessage;
		}
		else {
			return "";
	 ...
	 */
	/*
	public static String checkForValidPassword1(String input) {
		...
		if(input.length() <= 0) {
			...
			return "\n*** ERROR *** The input must be 255 characters or less";
		};
		...
		while (running) {
			...
			case 0: 
				...
				if ((currentChar >= 'A' && currentChar <= 'Z' ) ||		// Check for A-Z
						...
						(currentChar >= '0' && currentChar <= '9' ))	// check of 0-9
			...
			case 1: 
				...
				// . -> State 2
				else if (currentChar == '.' || currentChar == '_' || currentChar == '-') {			// Check for . _ or -/
					nextState = 2;
				...
				// The execution of this state is finished
				// If the size is larger than 255, the loop must stop
				if (passwordSize > 255)
					running = false;
				break;			
		...
		case 0:
			...
			passwordRecognizerErrorMessage += "A Password must start with A-Z or a-z.\n";
			return passwordRecognizerErrorMessage;

		case 1:
			// State 1 is a final state.  Check to see if the Password length is valid.  If so we
			// we must ensure the whole string has been consumed.

			if (passwordSize < 4) {
				// Password is too small
				passwordRecognizerErrorMessage += "A Password must have at least 4 characters.\n";
				return passwordRecognizerErrorMessage;
			}
			else if (passwordSize > 255) {
				// Password is too long
				passwordRecognizerErrorMessage += 
					"A Password must have no more than 255 characters.\n";
				return passwordRecognizerErrorMessage;
			}
			else if (currentCharNdx < input.length()) {
				// There are characters remaining in the input, so the input is not valid
				passwordRecognizerErrorMessage += 
					"A Password character may only contain the characters A-Z, a-z, 0-9.\n";
				return passwordRecognizerErrorMessage;
			}
			else {
					// Password is valid
		...
		case 2:
			...
			passwordRecognizerErrorMessage +=
				"A Password character after a period, dash, or underscore must be A-Z, a-z, 0-9.\n";
			return passwordRecognizerErrorMessage;
