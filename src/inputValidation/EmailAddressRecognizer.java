	...
	public static String checkEmailAddress(String input) {
		...
		while (running) {
			...
			case 1: 
				...
				// If it is '.', go back to state 0
				else if(currentChar == '.') {
					nextState = 0;
				}				
				// If it is '@', go to state 2
				else if(currentChar == '@') {
					nextState = 2;
				}
				// If it is none of those characters, the FSM halts
			...
			case 2: 
				...
				// If it is alphanumeric character go to state 3
				if((currentChar >= 'A' && currentChar <= 'Z')|| 		// Upper case
						(currentChar >= 'a' && currentChar <= 'z') ||	// Lower case
			...
			case 3:
				...
				// If it is '-' character go to state 4
				else if(currentChar == '-') {
					nextState = 4;
				}
				// If it is '-' character go to state 2
				else if(currentChar == '.') {
					nextState = 2;
				}
				else {
		...
		case 0:
			...
			emailAddressIndexofError = currentCharNdx;		// Copy the index of the current character;
			emailAddressErrorMessage = "Valid Email requires only alphanumberic or '-' or '.' before '@'.\\n";
		...
		case 1:
			...
			emailAddressIndexofError = currentCharNdx;		// Copy the index of the current character;
			emailAddressErrorMessage = "Valid Email requires exactly one '@'.\n";
			return emailAddressErrorMessage;
		...
		case 2:
			...
			emailAddressIndexofError = currentCharNdx;		// Copy the index of the current character;
			emailAddressErrorMessage = "Valid Email requires only alphanumberic or '-' or '.' after  @.\n";
			return emailAddressErrorMessage;
		...
		case 4:
			...
			emailAddressIndexofError = currentCharNdx;		// Copy the index of the current character;
			emailAddressErrorMessage = "Valid email must have alphanumberic after any '-'.\n";
			return emailAddressErrorMessage;
